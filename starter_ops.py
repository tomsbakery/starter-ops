...
def push_protections(owner, repo):
    """
    does the work of actually applying default branch protections to
    a particular repostory using the GitHub REST API
    """
    res = []
...
def oven():
    ...
    and calls the push_protections function which does the work
    """
    event_dict = request.json

    # GH will notify us on a few repo events, but we only take action when a repo is created
    ...
    return make_response(jsonify({"code": "success", "status": 200}), 200)

@app.route("/toaster/<string:owner>/<string:repo>", methods=["GET"])
def toaster(owner, repo):
    """
    receives an incoming GET request which specify a single repository
    and calls the push_protections function which does the work
    """
    return push_protections(owner, repo)
